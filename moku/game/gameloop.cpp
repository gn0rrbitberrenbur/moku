...
void game_loop_pvp(Board& board)
{
    ...
    while (true)
    {
        std::cout << "[info] Turn no. " << move_count << " " << (is_black_turn ? "Black" : "White")
                  << " turn." << std::endl;
...
void game_loop_minimax(Board& board)
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            int best_move =
                ai.get_best_move_timed_smp(board, is_black_turn, g_config.time_limit_ms);

            std::cout << "[info] AI plays: " << (char)('A' + best_move % SIZE)
                      << (best_move / SIZE + 1) << std::endl;
