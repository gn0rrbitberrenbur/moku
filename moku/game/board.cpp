...
/**
 * This file implements the Board class defined in ../include/board.hpp, which represents the game
 * state of Gomoku. The Board class uses two bitsets to track the positions of black and white
 * pieces on a 15x15 board.
 */

/**
 * Outputs the current state of the board to the console. Black pieces are represented by 'X', white
 * pieces by 'O', and empty spaces by '.'.
 * @param None
 * @return void
 */
...
/**
 * Undoes a move on the board at the specified position, clearing both black and white pieces from
 * that position.
 * @param pos The position on the board, represented as an index (0-224) where the move is to be
 * undone.
 * @return void
 */
 ...
 * @return boolean ; true if the position is occupied by either a black or white piece, false
 * otherwise
 */
...
/**
 * Checks if the given bitset (representing either black or white pieces) contains a sequence of
 * five consecutive pieces in any direction.
    ...
    for (int p = 0; p < squares(); p++)
    {
        ...
        for (int dir = 0; dir < 4; dir++)
        {
            ...
            for (int i = 1; i < 5; i++)
            {
                int nr = r + dy[dir] * i;
                int nc = c + dx[dir] * i;
                if (!in_bounds(nr, nc) || !pieces[pos(nr, nc)])
                    break;
                count++;
            }
