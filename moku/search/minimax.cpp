...
/**
 * This file implements the Minimax class defined in ../include/agent/minimax.hpp, which uses the
 * minimax algorithm with alpha-beta pruning to determine the best move for a given board state.
 */
...
int MinimaxAgent::get_best_move_timed(Board& board, bool is_black, int time_limit_ms)
{
    ...
    {
        std::cerr << "[info] Starting iterative deepening (limit: " << time_limit_ms
                  << "ms, moves: " << moves.size() << ")" << std::endl;
    }
    ...
    for (int depth = 1; depth <= max_depth; depth++)
    {
        ...
        for (size_t i = 0; i < moves.size(); i++)
        {
            ...
            {
                score =
                    is_black ? 100000.0f - (max_depth - depth) : -100000.0f + (max_depth - depth);
            }
            else
            {
        ...
        if (!search_aborted)
        {
            ...
            if (g_config.debug_output == true)
            {
                std::cerr << "[info] Depth: " << depth << " | Score: " << best_score
                          << " | Best Move: " << best_move << " | Nodes: " << nodes_searched
                          << " | Time: " << (int)(depth_time * 1000) << "ms" << std::endl;
            }
    ...
    if (g_config.debug_output == true)
    {
        std::cerr << "[info] Search complete: Depth: " << completed_depth
                  << " | Best Move: " << best_move << " | Time: " << (int)(total_time * 1000)
                  << "ms"
                  << " | Nodes: " << nodes_searched << " | TT Hits: " << tt_hits
                  << " | TT Size: " << tt.size() << std::endl;
    }
 ...
 * @param is_maximizing ; bool ; A boolean indicating whether the current player is the maximizing
 * player.
    ...
    if (is_maximizing)
    {
        ...
        if (!search_aborted)
        {
            int flag = (max_eval <= original_alpha) ? TTEntry::UPPERBOUND
                       : (max_eval >= beta)         ? TTEntry::LOWERBOUND
                                                    : TTEntry::EXACT;
    ...
    else
    {
        ...
        if (!search_aborted)
        {
            int flag = (min_eval <= original_alpha) ? TTEntry::UPPERBOUND
                       : (min_eval >= beta)         ? TTEntry::LOWERBOUND
                                                    : TTEntry::EXACT;
...
int MinimaxAgent::get_best_move(Board& board, bool is_black)
{
    ...
    int best_move = moves[0];
    float best_score =
        is_black ? -std::numeric_limits<float>::infinity() : std::numeric_limits<float>::infinity();
    ...
    if (g_config.debug_output == true)
    {
        std::cerr << "[info] Search complete: Depth: " << max_depth << " | Best Move: " << best_move
...
int MinimaxAgent::get_best_move_timed_smp(Board& board, bool is_black, int time_limit_ms)
{
    ...
    auto worker = [&](unsigned int thread_id)
    {
        ...
        for (int depth = start_depth; depth <= max_depth; depth++)
        {
            ...
            if (thread_id == 0 && g_config.debug_output)
            {
                ...
                std::lock_guard<std::mutex> lock(result_mutex);
                std::cerr << "[info] depth=" << depth << " | best=" << current_best_move
                          << " | score=" << current_best_score
                          << " | time=" << (int)(elapsed * 1000) << "ms"
                          << " | nodes=" << nodes_searched.load(std::memory_order_relaxed)
                          << " | tt_hits=" << tt_hits.load(std::memory_order_relaxed) << std::endl;
            }
    ...
    if (g_config.debug_output)
    {
        ...
        std::cerr << "[info] SMP search: threads=" << threads
                  << " | depth=" << completed_depth.load() << " | best=" << shared_best_move.load()
                  << " | time=" << (int)(total_time * 1000) << "ms"
                  << " | nodes=" << nodes_searched << " | tt_hits=" << tt_hits << std::endl;
    }
