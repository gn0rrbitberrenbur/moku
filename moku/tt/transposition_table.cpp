 ...
 * provides a hash-based caching mechanism for game states in a Gomoku AI. The class uses Zobrist
 * hashing to efficiently compute and update hash values for board states, allowing for quick
 * lookups and storage of evaluated positions.
 */
...
uint64_t TranspositionTable::compute_hash(const Board& board, bool is_black_turn) const
{
    ...
    {
        if (board.black[i])
            hash ^= zobrist_black[i];
        if (board.white[i])
            hash ^= zobrist_white[i];
    }
...
/**
 * This function probes the transposition table for a given hash and retrieves the corresponding
 * entry if it exists.
 * @param hash The Zobrist hash of the board state to probe.
 * @param entry A reference to a TTEntry structure where the retrieved entry will be stored if
 * found.
