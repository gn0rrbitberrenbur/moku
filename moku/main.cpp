...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if ((strcmp(argv[i], "--cores") == 0 || strcmp(argv[i], "-c") == 0) && i + 1 < argc)
        {
            ...
            {
                std::cerr << "[warning] Requested cores exceed hardware concurrency, using max "
                             "available cores"
                          << std::endl;
    ...
    if (!engine_mode)
    {
        std::cout << "[info] Config: size=" << g_config.board_size << "x" << g_config.board_size
                  << ", depth=" << g_config.max_depth << ", time=" << g_config.time_limit_ms << "ms"
