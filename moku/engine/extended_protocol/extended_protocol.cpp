...
void Engine::cmd_turn(std::istringstream& iss)
{
    ...
    {
        std::cerr << "[DEBUG TURN] pos=" << pos << " board_size=" << g_config.board_size
                  << std::endl;
    }
    ...
    if (g_config.debug_output == true)
    {
        std::cerr << "[DEBUG TURN] is_black=" << is_black << " black_count=" << board.black.count()
...
void Engine::cmd_board()
{
    ...
    bool we_are_black =
        (own_moves.size() > opp_moves.size()) || (own_moves.size() == opp_moves.size());
...
/**
 * This function processes the TAKEBACK command, which allows undoing a move at a specified
 * position. TAKEBACK X,Y - Undo move at position Must respond with OK or ERROR
 * @param iss An input string stream containing the command arguments
 * @return void
 */
